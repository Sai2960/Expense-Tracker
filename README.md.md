...
```
> ⚠️ Important: `REACT_APP_API_URL` must include the full path `/api/expenses`, not just the base domain. The frontend uses this value directly in its `axios` calls (e.g. `axios.get(API_URL)`), so a bare domain will hit the wrong route.
```bash
npm start
```
...
|---|---|---|---|
| Frontend | [Vercel](https://vercel.com) | https://expense-tracker-kappa-one-25.vercel.app | Root directory set to `frontend`; `REACT_APP_API_URL=https://expense-tracker-vg0n.onrender.com/api/expenses` |
